# criar_painel_safra_v2.py
# FASE 2C — Painel Safra / Portal Operações RNO
# Execute na RAIZ do projeto:
# cd "C:\Users\n5996917\OneDrive - Claro SA\INTRANET\OPERACOES_RNO"
# python criar_painel_safra_v2.py
...
REL = ROOT / "relatorio_criar_painel_safra_v2.txt"
SNIP = ROOT / "snippet_registro_dash_safra.txt"
...
def _conn():
    for mod, fn in [("data.db", "get_connection"), ("data.db", "get_db_connection"), ("data.db", "conectar")]:
        try:
    ...
    return mysql.connector.connect(host="localhost", user="root", password="", database="operacoes_rno")
...
def _pick(cols, nomes):
    mapa = {str(c).upper(): c for c in cols}
    for n in nomes:
        if str(n).upper() in mapa:
            return mapa[str(n).upper()]
    return None


def _canon_safra(v):
    s = str(v or "").upper().strip()
    s = s.replace("º", "").replace("°", "").replace("MÊS", "MES").replace("MÊSES", "MESES")
    if "13" in s:
        return "13 MESES"
    if "4" in s:
        return "4 MESES"
...
def _canon_tipo(v):
    s = str(v or "").upper().strip()
    for a, b in [("Ç", "C"), ("Ã", "A"), ("Á", "A"), ("À", "A"), ("Â", "A"), ("Í", "I"), ("Ê", "E"), ("É", "E"), ("Ó", "O")]:
        s = s.replace(a, b)
    if "OP" in s:
        return "OPCAO"
    if "INAD" in s:
...
def _where(cols, args, diario=False):
    mapa = {
        ...
        "uf": ["UF"],
        "cidade": ["CIDADE", "NM_CIDADE", "OPERACAO", "OPERAÇÃO"],
        "safra": ["SAFRA"],
        "tipo": ["TIPO", "DS_TIPO_DESCONEXAO", "TIPO_DESCONEXAO"],
    }
    wh, params = [], []
...
def _load(table, args, diario=False):
    ...
    c_uf = col(["UF"])
    c_cidade = col(["CIDADE", "NM_CIDADE", "OPERACAO", "OPERAÇÃO"])
    c_safra = col(["SAFRA"])
    c_tipo = col(["TIPO", "DS_TIPO_DESCONEXAO", "TIPO_DESCONEXAO"])
    c_desc = col(["DESC", "DESCONECTADOS", "TOTAL_DESC", "QTD_DESC", "VOLUME_DESC", "TOTAL_DESCONECTADO", "TOTAL_DESCONECTADOS"])
    c_rec = col(["REC", "RECUPERADOS", "TOTAL_REC", "QTD_REC", "VOLUME_REC", "TOTAL_RECUPERADO", "TOTAL_RECUPERADOS"])
...
def _metas():
    ...
    try:
        ...
        c_tipo = _pick(df.columns, ["TIPO", "DS_TIPO_DESCONEXAO", "TIPO_DESCONEXAO"])
...
def _payload(df):
    ...
    return {"ok": True, "kpis": {"desc": int(round(total_desc)), "rec": int(round(total_rec)), "pct": _pct(perc), "meta": _pct(meta_pond), "gap_vol": int(round(gap_vol)), "gap_pct": _pct(gap_pct), "faltam": int(math.ceil(faltam)), "necessario_dia": int(math.ceil(faltam)), "tendencia": "ACIMA DA META" if gap_pct >= 0 else "ABAIXO DA META"}, "matriz": matriz, "ranking": ranking, "comparativo": comparativo, "tipos": tipos}
...
def localizar_app():
    candidatos = [ROOT/"app.py", ROOT/"main.py", ROOT/"run.py", ROOT/"wsgi.py", ROOT/"application.py", ROOT/"server.py", ROOT/"__init__.py"]
    for p in ROOT.glob("**/__init__.py"):
        if "venv" not in str(p).lower() and "site-packages" not in str(p).lower() and p not in candidatos:
            candidatos.append(p)
    achados = []
    for p in candidatos:
        if not p.exists():
        ...
        if "dash_log" in txt or "dash_retirada" in txt or "dash_quebra" in txt or "dash_backlog" in txt: score += 8
        if score > 0:
            achados.append((score, p, txt))
    achados.sort(reverse=True, key=lambda x: x[0])
    return achados


def registrar():
    SNIP.write_text(IMPORT_LINE + "\n" + REGISTER_LINE + "\n", encoding="utf-8")
    apps = localizar_app()
    if not apps:
        return "NÃO REGISTRADO — arquivo principal Flask não localizado. Use snippet_registro_dash_safra.txt."
    _, alvo, txt = apps[0]
    if "dash_safra" in txt and "register_blueprint(dash_safra_bp" in txt:
        return f"JÁ REGISTRADO em {alvo}"
    backup(alvo)
    ...
    if pos_reg is not None:
        indent = re.match(r"^(\s*)", linhas[pos_reg]).group(1)
        linhas.insert(pos_reg + 1, indent + REGISTER_LINE)
    else:
        pos_return = None
        for i, l in enumerate(linhas):
            if re.search(r"\breturn\s+app\b", l):
                pos_return = i
        if pos_return is not None:
            indent = re.match(r"^(\s*)", linhas[pos_return]).group(1)
            linhas.insert(pos_return, indent + REGISTER_LINE)
        else:
            return f"NÃO REGISTRADO — ponto seguro não localizado em {alvo}. Use snippet_registro_dash_safra.txt."
    alvo.write_text("\n".join(linhas) + "\n", encoding="utf-8")
    return f"REGISTRADO em {alvo}"
...
def main():
    ...
    status = registrar()
    REL.write_text("\n".join([
        "RELATÓRIO — CRIAR PAINEL SAFRA V2",
        f"Data/hora: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"Raiz: {ROOT}",
        f"dash_safra.py: {DASH} — EXISTE={DASH.exists()}",
        f"dash_safra.html: {HTML} — EXISTE={HTML.exists()}",
        f"Registro Flask: {status}",
        "Backups: " + (", ".join(backups) if backups else "nenhum"),
        "Rotas: /dash/safra/ | /dash/safra/api/options | /dash/safra/api/data",
        "Reinicie o Flask antes de testar."
    ]), encoding="utf-8")
    print("OK — Painel Safra criado/registrado.")
    print("dash_safra.py:", DASH)
    print("dash_safra.html:", HTML)
    print("Registro:", status)
    print("Relatório:", REL)
    print("Agora reinicie o Flask e teste /dash/safra/.")
